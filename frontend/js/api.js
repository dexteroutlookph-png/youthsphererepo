...
const DEV_API_PORTS = [5000, 5001, 5002, 5003, 5004, 5005];
...
const api = {
  ...
  async request(endpoint, options = {}) {
    ...
    let lastError = null;
    const apiUrls = isLocalDevelopment
    ...
    for (const url of apiUrls) {
      try {
        ...
        if (!response.ok) {
          lastError = new Error(data.message || 'An error occurred during request execution.');
          continue;
        }
